...
| Wikipedia/Wikidata taxonomy (`wikipedia-taxonomy`, `data/etl/sources/wikipedia_taxonomy.py`) | https://en.wikipedia.org/ (page lookup) + https://www.wikidata.org/ (P171/P105 parent-taxon walk) | `family`/`genus` backfill for plants no other source had taxonomy for (~54 of 359 after the main run, mostly cultivars) - looked up per-genus (derived from `botanical_name`'s first word), not per-plant, since cultivars of one species share a genus. See `data/etl/backfill_taxonomy.py`, a separate entrypoint from `etl.run` (operates on already-exported `data/plants/*.json`, same reasoning `growing_info/run.py` documents for being separate) | CC BY-SA (Wikipedia) / CC0 (Wikidata) | built, not yet run |
...
- **`bedding_needs`** (hilling, ground cover, staking, ...) - also how-to-grow content rather than a database field. General gardening how-to sites/extension publications are the likely source; alternatively, `growth_habit` (see below) might let Ollama *infer* staking needs for vining/climbing plants even without an explicit field.

**Correction (2026-07-18, data-engineer agent):** an earlier version of this note claimed Trefle has `growth_form`/`growth_habit`/`shape_and_orientation` fields at the top level of `main_species`. Those top-level keys exist but are always `null` in every cached response checked. The real, populated data is one level deeper: `main_species.specifications.growth_habit` - a comma-separated free-text field (USDA-style values: `"Forb/herb"`, `"Tree"`, `"Shrub"`, `"Vine"`, `"Subshrub"`, `"Graminoid"`, and combinations) with real data cached for ~216 of 359 plants. `main_species.specifications.average_height`/`maximum_height` also exist but are mostly `null` in what's cached - not reliable, don't use them for sizing (use `Plant.height_cm`/`spread_cm` instead, sourced elsewhere). This is what backs the project's own `growth_habit` field on `Plant` (see `plant.schema.json`) - a small rendering-oriented category set (`upright`/`spreading`/`climbing`/`rosette`/`tree`), not Trefle's raw combinatorial string - populated by `data/etl/populate_growth_habit.py`.
- **`life_cycle`/`life_cycle_years`** (added after the main ETL run already completed - none of the 359 exported plants have it yet) - `life_cycle` (annual/biennial/perennial) is common structured-database data, worth checking whether Trefle's `main_species` has an unmapped duration-type field (`sources/trefle.py`'s `map_record` doesn't currently read one - verify against a real cached response in `data/.cache/trefle/`, not just the docs page, per the existing Trefle gotcha above) before assuming a new source is needed. `life_cycle_years` (a perennial's typical productive lifespan, e.g. 7 for raspberries) is more specialized - likely needs `growing_information`/extension-publication text like the other unstructured-only fields above, not a structured API.
...
**Status (updated 2026-07-20): built and run to completion.** This note originally said "schema only, not wired up yet" - that's stale. `data/etl/growing_info/` (`fetch.py`/`split.py`/`match.py`/`passes.py`/`run.py`/`state.py`/`storage.py`) implements the full pipeline described below and has been run against all 7 books: `etl.state_report` confirms 140/140 matched plants with all 4 Ollama passes done (consolidate/extract/crosscheck/surface), 521 sections checkpointed. `data/plants/*.json` files that matched a book section now carry real `growing_information` entries (verified directly, e.g. `tomato.json`). Output logs: `data/growing_info_unmatched.jsonl` (29 unmatched sections), `data/growing_info_crosscheck_log.jsonl` (104 entries), `data/growing_info_interesting_log.jsonl` (611 entries) - see `data/task_queue.md`'s item 3 for the full run history. Also now wired into `backend/app/scripts/import_plants.py` (task_queue item 6) - not yet run against real Postgres by anyone, per that item's own note.
