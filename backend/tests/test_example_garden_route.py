...
def test_get_example_garden_returns_the_new_field_row_planting_shape(
    client: TestClient, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """#243: the real data/example_garden.json fixture (regenerated by #234)
    emits plant_slug/placement_type/geometry/spacing_cm per planting, not
    the old flat x_cm/y_cm point - GET /api/example-garden must validate
    (not 500 on) that shape."""
    _write_fixture(
        tmp_path,
        monkeypatch,
        [
            {
                ...
                "plantings": [
                    {
                        "plant_slug": "brandywine-tomato",
                        "placement_type": "field",
                        "geometry": _rect(0, 0, 100, 50),
                        "spacing_cm": 50.0,
                    }
                ],
            }
    ...
    assert planting["plant_slug"] == "brandywine-tomato"
    assert planting["placement_type"] == "field"
    assert planting["geometry"]["type"] == "rectangle"
    assert planting["spacing_cm"] == 50.0
