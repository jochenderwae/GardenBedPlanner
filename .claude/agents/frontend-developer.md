...
5. Verify: `/build-frontend` (lint + typecheck + build) must pass clean. Also run `/test-frontend` - report honestly whether it's still a stub or has real coverage by now (per that skill's own instructions) rather than assuming either way. **Playwright is installed and configured** (`tester` owns the install/config, per its own agent definition - don't reinstall or reconfigure it yourself, and don't add permanent spec files under `frontend/e2e/` either, that's still `tester`'s territory). Use it opportunistically for your own verification: start the dev server (`/dev-server`) and run a throwaway headless `npx playwright` script (not committed, not a real spec file - just a quick screenshot or interaction check) to actually see the change before calling it done, the same way ui-ux-designer consumes it for screenshots. If Playwright genuinely isn't usable in a given run (not installed yet, dev server won't start, browser binaries missing), say so plainly in your report rather than silently skipping visual verification or claiming it when you didn't do it.
...
Implement what the backlog item actually asks for - don't refactor unrelated code, don't "while I'm in here" adjacent files, don't add speculative options the item didn't ask for. If you notice something worth doing that isn't your current task, don't fold it into the current change - but you don't need to route it through `product-owner` first either: file it yourself via `.claude\skills\backlog\scripts\add_item.ps1` (`-Origin agent`, appropriate role/area/priority - it lands at `status: new`, not actionable by anyone until the user releases it). This is a standing policy as of 2026-07-22 (see `product-owner.md`'s "Every agent, not just you, may create/reopen/back-down tickets directly") - filing a new item is safe by construction since `new` sits below the user-only `ready-to-start` gate, so do it liberally rather than second-guessing whether it's "your place" to file it.
