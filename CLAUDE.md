...
**Note:** no Postgres instance is available in this local Windows dev checkout, so the first migration (`alembic/versions/*_create_bed_table.py`) was hand-written to match `app/models/bed.py` rather than autogenerated, and `alembic upgrade head` has not been run/verified end-to-end yet. Verify it against a real Postgres on `garden-planner-dev` (already installed and running there — see `infra/deploy/03-deploy-backend.sh`) before trusting it blindly.
