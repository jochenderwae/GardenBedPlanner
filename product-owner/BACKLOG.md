...
- [ ] WYSIWYG bed/layout canvas editor (`react-konva`) - `frontend/src/pages/Layout.tsx` currently renders one static placeholder dashed rectangle, not connected to the Bed API or draggable/resizable at all. **In progress as of 2026-07-18**: build started (Bed-level canvas editor wired to the real Bed CRUD API), building against a phased spec (see product-owner/research/wysiwyg-bed-editor.md) - phase 0 (bed-level drag/resize/CRUD) now, plant placement/rotation/domain-intelligence overlays deliberately deferred to later phases pending `BedPlanting`/`growth_habit`/`Bed.orientation`.
- [ ] `BedPlanting` model + API - plant placements on a bed (individual/row/field, geometry, planted/removed dates) per `docs/schema.md`; no backend model yet. Needs a per-plant rendering geometry (shape/size) to actually draw placements on the canvas - see the Plant database section's `growth_habit` item, which `data-engineer` is producing the data for (schema/data side only; a `Plant.growth_habit` column + API + frontend consumption is still someone's to build once that data exists).
- [ ] Installation-time choice to load example/demo garden data - user request (2026-07-18): let the user decide whether to seed the app with realistic example data (beds + plant placements) on first install, rather than always starting empty or always seeding. Explicitly **not being implemented now** - just the choice itself is backlogged; the example dataset it would load (`data/example_garden.json`) is being generated now as a `data-engineer` task so it exists once this gets built. Needs: a seed-import script (mirrors `backend/app/scripts/import_plants.py`'s pattern, but for beds/plantings once `BedPlanting` exists), and an onboarding UI moment to ask the question.
