...
- **Self-updating-script gotcha: any run that pulls a new commit of itself mid-execution keeps running the *old* in-memory copy for the rest of that invocation.** `03`/`04` both `git pull` the repo they live in as their first step. Bash doesn't re-read a script from disk mid-run — it keeps its original file handle, and `git pull`'s checkout replaces the file via rename, so the already-running process never sees the update. This has caused two real failures: once where the pulled fix to `03` itself (the `uv` PATH export) didn't take effect until the *next* invocation, and once where an old `sed` (missing a new `-e` substitution rule) ran against an already-updated `.template` file and left a literal `__PLACEHOLDER__` token in the installed config. **Rule of thumb: after pushing any fix to a script that's about to be re-run, expect to invoke it twice** — the first run only pulls the fix; the second run is the first one that actually executes it consistently. Don't diagnose a fresh bug before checking whether this is just round one.
